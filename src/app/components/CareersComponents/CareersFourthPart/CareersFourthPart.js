...
function CareersFourthPart (){
    ...
    const renderOffers=offers.map((item, index)=>(
        <div className={classes.offersWhole} key={index}>
            <div className={classes.offersImgDiv}><img className={classes.offersImg} src={item.image}/></div>
            <div className={classes.offersText}>{item.text}</div>
        </div>
    ))
